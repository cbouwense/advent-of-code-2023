 ...
 * "asdf" --> ""
 */
fn extract_numbers_from_line(line: &&str) -> String {
...
/**
 * "" -----> ""
 * "1" ----> "11" (two 1s here is intentional)
 * "12" ---> "12"
 * "123" --> "13"
 */
...
fn part_one(input: &[&str]) -> i32 {
    let only_numbers: Vec<String> = input
        .iter()
        .map(extract_numbers_from_line)
        .collect();

    let first_and_last_number_strs: Vec<String> = only_numbers
        .iter()
        .map(first_and_last)
        .collect();

    let first_and_last_nums: Vec<i32> = first_and_last_number_strs
        .iter()
        .map(|s| s.parse::<i32>().unwrap_or(0))
        .collect();
    
    first_and_last_nums.iter().sum::<i32>()
}
