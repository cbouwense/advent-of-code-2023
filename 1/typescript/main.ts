...
const isDigit = (s: string) => Object.keys(wordToDigit).includes(s);

const translateWordsToDigits = (line: string) => {
    let result = '';

    for (let i = 0; i < line.length; i++) {
        const nextThreeCharacters = line.slice(i, i+3);
        if (nextThreeCharacters.length === 3 && isDigit(nextThreeCharacters)) {
            result += wordToDigit[nextThreeCharacters];
            continue;
        }
        
        const nextFourCharacters = line.slice(i, i+4);
        if (nextFourCharacters.length === 4 && isDigit(nextFourCharacters)) {
            result += wordToDigit[nextFourCharacters];
            continue;
        }

        const nextFiveCharacters = line.slice(i, i+5);
        if (nextFiveCharacters.length === 5 && isDigit(nextFiveCharacters)) {
            result += wordToDigit[nextFiveCharacters];
            continue;
        }
...
const partTwo = (lines: string[]) => partOne(lines.map(translateWordsToDigits));
