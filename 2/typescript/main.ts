export const gameNumber = (line: string) => {
    if (line.length === 0) return 0;

    const gameAndNumber = line.split(':')[0];
    const numberAsString = gameAndNumber.split(' ').at(-1);

    return Number(numberAsString);
}

export const parseHandfullsFromGame = (line: string) => {
...
export const colorCount = (game: string, color: Color) => {
    if (game.length === 0) return 0;

    const countAndColor = game
        .split(', ')
        .filter(t => t.includes(color));
    
    if (countAndColor.length === 0) return 0;

    return Number(countAndColor[0].split(' ')[0]);
}
...
const partOne = (input: string[]) => {
    ...
    return input
        .map((line) => {
            const gameNo = gameNumber(line);
            const handfullsFromGame = parseHandfullsFromGame(line);
            
            let isGamePossible = true;
            handfullsFromGame.forEach(handfull => {
                if (colorCount(handfull, 'red') > MAX_RED)     isGamePossible = false;
                if (colorCount(handfull, 'green') > MAX_GREEN) isGamePossible = false;
                if (colorCount(handfull, 'blue') > MAX_BLUE)   isGamePossible = false;
            });

            return isGamePossible ? gameNo : 0;
        })
...
const partTwo = (input: string[]) => {
    return input
        .map((line) => {
            ...
            handfullsFromGame.forEach(handfull => {
                if (colorCount(handfull, 'red') > maxRed)
                    maxRed = colorCount(handfull, 'red');
                if (colorCount(handfull, 'green') > maxGreen)
                    maxGreen = colorCount(handfull, 'green');
                if (colorCount(handfull, 'blue') > maxBlue)
                    maxBlue = colorCount(handfull, 'blue')
            });
